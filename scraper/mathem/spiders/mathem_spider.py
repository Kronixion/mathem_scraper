...
class MathemSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.mathem.se/se/"]
    ...
    def parse(self, response: Response) -> Iterator[scrapy.Request]:
        ...
        categories = list(self._extract_category_slugs(build_data))
        if not categories:
    ...
    def _extract_category_slugs(self, build_data: dict) -> Iterable[str]:
        """Pull category slugs from the landing page dehydrated state."""

        queries = (
            build_data.get("props", {})
            .get("pageProps", {})
            .get("dehydratedState", {})
            .get("queries", [])
        )
        for query in queries:
            ...
            for block in data.get("blocks", []):
                if block.get("component") != "megamenu-category-blocks":
                    continue
                for item in block.get("items", []):
                    payload = item.get("data", {}) if isinstance(item, dict) else {}
                    target = payload.get("target", {})
                    uri = target.get("uri")
                    if uri and uri.startswith("/se/categories/"):
                        slug = uri.split("/se/categories/")[-1].strip("/")
                        if slug:
                            yield slug
